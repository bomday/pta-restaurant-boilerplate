...
export default class RestaurantController implements Crud {
    ...
    async create(request: Request, response: Response){
        const {restaurantName, restaurantStreet, foodStyle} = request.body;

        const isAnyUndefined = Citi.areValuesUndefined(restaurantName, restaurantStreet, foodStyle);
        if(isAnyUndefined) return response.status(400).send();

        const newRestaurant = {restaurantName, restaurantStreet, foodStyle};
    ...
    async update(request: Request, response: Response){
        ...
        const restaurantWithUpdatedValues = {restaurantName, restaurantStreet, foodStyle};
